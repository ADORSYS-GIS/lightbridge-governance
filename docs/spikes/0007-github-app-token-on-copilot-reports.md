...
- Status: Findings recorded; decision made. Empirical run pending (see "Verification evidence").
...
| Org policy toggle state | **OFF, empirically.** Live call 2026-08-02: `GET /orgs/adorsys-gis/copilot/metrics/reports/organization-1-day` with a `read:org` PAT returned `403 {"message":"The 'Copilot usage metrics' policy must be enabled to use this API"}`. Blocks App tokens and PATs equally. | Live `curl` (evidence appended to [#5](https://github.com/ADORSYS-GIS/lightbridge-governance/issues/5)) |
