...
# Brace-match `#[cfg(test)]` blocks and emit `path\ttotal\tprod\ttest` per file.
# Paths arrive as argv (NUL-safe via xargs -0). The counter is a small state
# machine:
...
for path in sys.argv[1:]:
    ...
    print(f"{path}\t{total}\t{prod}\t{test}")
PY
  xargs -0 python3 "${tmp}"
  rm -f "${tmp}"
}

# Exclude test-only files from the burn-down list: integration-test directories
# (`/tests/`) and extracted inline-test modules (`tests.rs`).
is_test_only() {
  case "$1" in
...
find "${roots[@]}" -name '*.rs' -not -path '*/target/*' -print0 \
  | count \
  | awk -v t="${THRESHOLD}" '$2 > t { print }' \
  | while IFS=$'\t' read -r path total prod test; do
      is_test_only "${path}" || printf '%s\t%s\t%s\t%s\n' "${path}" "${total}" "${prod}" "${test}"
    done \
  | sort -t $'\t' -k3,3nr \
