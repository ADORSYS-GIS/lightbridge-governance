...
/// The durable spool checkpoint's `discarded_total`, read directly from the
/// state directory -- there is no `status` surface for it yet (that is
/// #271's dashboard, not this story), so the test reads what the daemon
/// itself persisted.
fn discarded_total(harness: &Harness) -> Result<u64> {
    let path = harness.state_dir().join("otel-daemon-checkpoint.json");
