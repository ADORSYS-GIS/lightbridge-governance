...
//! now supplies that part to both). Past that they diverge because the two
...
pub struct Checkpoint {
    ...
    /// separate attempts. Never bytes lost to an outage: an unreachable
    /// collector leaves the offset exactly where it was, and the same bytes
    /// are retried, not counted here.
    #[serde(default)]
...
/// Writes tmp-then-rename so a reader never sees a half-written offset.
