...
/// full power loss, which is not the failure this daemon exists to survive
/// (#269's AC1 is explicitly "kill -9, restart" -- a process death, not a
/// power loss -- but `serve()`'s own module doc also promises "a killed
/// laptop loses at most the narrow, documented exception," which a
/// page-cache-only write cannot honour). The two writers make the opposite,
/// each-defensible call the review brief that found this asked for: pick one
/// and say which. This one fsyncs; Copilot's journal does not, because
/// Copilot's spool is Chat's own file, rewritten by VS Code independent of
/// this binary, so surviving *this process's* death was never the property
/// being bought there.
#[cfg(unix)]
