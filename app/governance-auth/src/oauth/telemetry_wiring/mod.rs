...
//! file in the tree. This module split into its own directory for the same
//! reason, once #272 gave it a second Copilot-path field and its tests.
...
pub(super) struct TelemetryWiring {
    ...
    /// `true` under `daemon` with an endpoint configured (issue #272 AC3):
    /// Copilot points its OWN `otlp-http` exporter at the loopback daemon,
    /// needing no credential at all -- the reason that exporter was
    /// abandoned in favour of the file one (a static header syncing
    /// off-machine via Settings Sync) does not apply to a loopback endpoint
    /// nothing off-machine can reach. Mutually exclusive with
