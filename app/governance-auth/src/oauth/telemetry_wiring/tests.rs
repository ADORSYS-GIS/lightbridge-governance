...
/// loopback value -- Claude Code and Codex still need it -- so the fix
/// is a second signal, not touching `endpoint`. #272 gave that signal a
...
/// #272's mutual-exclusion invariant, pinned directly: exactly one
/// Copilot path is active per profile whenever an endpoint is
/// configured, never both (which would leave Copilot's own config
/// ambiguous) and never neither (which would leave the spool undrained,
/// the bug the previous test's doc names).
#[test]
