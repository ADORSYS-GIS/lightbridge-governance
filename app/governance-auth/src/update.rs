...
//!
//! [`run`] tries [`OauthConfigArgs::resolve`] after a real (non-dry-run)
//! update installs successfully, and runs `configure` again only if that
//! resolves -- i.e. only on a machine that was already onboarded, where a
//! config file (or the caller's own flags/env) already supplies
//! `issuer`/`client_id`. A resolve failure is treated as "nothing to
//! re-apply", not as an update failure: this module's whole reason for
//! resolving nothing else is that "the machine most likely to be updating
//! is the one with no config yet" (see `Cli::run`'s doc), and that stays
//! true here -- a fresh machine's `self update` behaves exactly as before.
...
pub async fn run(http: &reqwest::Client, oauth: OauthConfigArgs, check_only: bool) -> Result<()> {
    ...
    reapply_configure_if_onboarded(oauth);
...
fn reapply_configure_if_onboarded(oauth: OauthConfigArgs) {
