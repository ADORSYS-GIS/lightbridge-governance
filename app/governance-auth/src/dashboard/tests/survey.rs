...
#[test]
fn a_codex_auth_command_with_embedded_arguments_is_stale() {
    let home = crate::managed::testutil::tempdir();
    seed_codex_auth(
        home.path(),
        "/usr/local/bin/governance-auth --issuer https://issuer.example --client-id client token",
        None,
    );
    assert!(
        survey(home.path()).stale,
        "the old one-string Codex invocation cannot be spawned"
    );
}

#[test]
fn a_codex_auth_command_with_separate_current_args_is_not_stale() {
    let home = crate::managed::testutil::tempdir();
    seed_codex_auth(
        home.path(),
        "/somewhere else/governance-auth",
        Some(&[
            "--issuer",
            "https://other.example",
            "--client-id",
...
fn seed_codex_auth(home: &std::path::Path, command: &str, args: Option<&[&str]>) {
