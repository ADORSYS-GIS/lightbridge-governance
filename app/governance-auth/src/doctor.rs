...
mod tests {
    ...
    // Everything above `report` in this file mints real credentials and makes
    // real HTTP requests -- covered by manual verification against a real
    // config (see this commit's own verification notes), not mocked here, the
    // same boundary `update.rs`'s own tests draw around `install`/`download`.
    // `report`'s pass/fail aggregation is pure and is what these tests cover.
