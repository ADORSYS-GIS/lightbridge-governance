//! Integration tests for the day-report path of `run_backfill_at` (the
//! scheduled backfill), against a mock GitHub and a real Postgres.
...
/// The ADR-0014 cutover freeze: with `CUTOVER_FREEZE_WRITES` on but no OTLP
/// sink configured, a run must fail loudly rather than silently write
/// nothing -- the whole point of the freeze is that the sink is the write
/// path, so a freeze with no destination is a misconfiguration, not a
/// graceful no-op.
#[tokio::test]
async fn freeze_without_a_sink_fails_loudly() {
    let Some(pool) = db_pool().await else {
        return;
    };
    let tenant_id = format!("it-ctl-freeze-nosink-{}", std::process::id());
    let org = "it-org-freeze-nosink";
    ...
    assert!(
        result.exit_result().is_err(),
        "a freeze with no sink must fail the run loudly"
    );
    assert!(
        result
            .failed
            .iter()
            .any(|(_, e)| e.contains("no OTLP sink")),
        "the freeze-without-sink error must be surfaced, not swallowed: {:?}",
        result.failed
    );
}
