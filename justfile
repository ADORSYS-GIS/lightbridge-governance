...
# Render the aiCliOtel collector and assert its OIDC auth gate survives, plus
# (lightbridge-governance#284) that client.address resolves from the
# rightmost X-Forwarded-For entry and a spoofed leading entry never survives.
# Deliberately NOT part of `all-checks`: it needs helm, mikefarah yq and
# Docker, which a Rust-only contributor may not have. CI runs it in the
# `helm-charts` job. `helm dependency build` runs first because the chart
# pulls the external `app-template` dependency (bjw-s), without which `helm
# template` errors out.
chart-checks:
